...
每次通用改动的完成条件是：代码与相关测试完成、中文手册更新、CHANGELOG 更新、必要时调整 roadmap，再同步独立源码。已发布安装包保持不变；需要新安装包时提升版本并重新构建、验证。
...
轨道、按钮与默认值变动必须同次更新 [中文使用手册](UserGuide.zh-CN.md)；规划变动更新 [roadmap](../ROADMAP.zh-CN.md)。新版本的历史说明写入 CHANGELOG，旧版本制品保持不可变。
